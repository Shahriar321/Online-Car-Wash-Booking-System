...
<div id="middle-div">
  ...
  <div id="content" class="mobileUI-main-content">
    <div class="registerDiv">
      <img src="images/pic7.jpg" class="loginBackgroundImg" id="loginBackgroundImg" width="100%">
...
<script>
  function checkTel(num) {
    regex = /^(?:\+?(88))? ?(?:\((?=.*\)))?(0?[1-9])\)? ?(\d\d\d(?:[- ](?=\d{3})|(?!\d\d[- ]?\d[- ]))\d\d[- ]?\d[- ]?\d{3})$/;
  ...
  var check = new Vue({
    ...
    methods: {
      checkEmail: function () {
        value = this.email;
        if (!value) return ""; 
        var myReg = /^[a-zA-Z0-9_-]+@([a-zA-Z0-9]+\.)+(com|cn|net|org|bd)$/;
        if (myReg.test(value)) {
